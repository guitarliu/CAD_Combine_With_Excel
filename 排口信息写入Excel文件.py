...
def get_and_write_data(file_name, excel_name, sheet_num):
    ...
    for text in msp.query("TEXT"):
        if text.dxf.layer == "ADD_PK":
            ...
            node_num = text_content.split("/")[3]
            text_info = text_name, text_x_cood, text_y_cood, pipe_diameter, elevation, node_num
            print(text_info)
...
for root, dirs, files in os.walk(os.getcwd()):
    for filename in files:
        ...
        if ".dxf" in filename:
            ...
            get_and_write_data(filename, excel_name, int(re.findall(r"\d*", filename)[5])-1)
